...
public class PawnListener implements ActionListener {
	...
	int captureFurth (int n, Field virtF, int i) {
    	if (virtF.neighbours[i]==Board.nullField||virtF.neighbours[i].neighbours[i]==Board.nullField) {
    		return n;				//0 - brak biæ
    	}
    	else {
    		if (virtF.neighbours[i].hasPawn&&(((Pawn)virtF.neighbours[i].getComponent(0)).side!=thisP.side)) {
    			if (!virtF.neighbours[i].neighbours[i].hasPawn) {
    				nextCaptures (n, virtF.neighbours[i].neighbours[i], i);
    			}
    		}
    	}
	...
	void nextCaptures (int n, Field virtF, int i) {
		captureFound(n+1, i, virtF.neighbours[i].neighbours[i]);
		if (virtF.neighbours[i]!=Board.nullField)
			nextCaptures(n, virtF.neighbours[i], i);
	}
}
